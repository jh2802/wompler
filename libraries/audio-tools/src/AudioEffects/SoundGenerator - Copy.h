...
/*
    ...
    T IRAM_ATTR readSample() override {
      ...
      if (sound_index > end_Index) {
	   ...
	   if ((sound_index <= loop_start)) {
		    if ((loop_mode==REVERSELOOP)){
		   sound_index=loop_end;
      ...
      T result = 0;
      if (this->is_running) {
        result = table[(sound_index>>SHIFTER)];
		if (delay>0){result=(int)(result+(table[(sound_index>>SHIFTER)+delay]))>>1;};
		
        if (reverse==false){sound_index+=index_increment;} else {int next=sound_index-index_increment;if ((next>start_Index) or (next<87831)){sound_index=next;};result*=-1; }
      }
	...
	void IRAM_ATTR setDelay(byte samples){
		delay=samples;
