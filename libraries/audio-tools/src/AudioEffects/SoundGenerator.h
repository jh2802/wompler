...
#define PINGPONG 4
#include "C:\Users\JJ Hardt\Documents\Arduino\streams_generator_i2s\sqlw.h"
// modified by Jack Hartman 2023-2026
/** 
...
newNote=false;
      ...
      //  } 
		...
		//o2_index=o2i;
					...
					prev_val[0]=result;
					if (rz==true){result=0;sound_index=start_Index;}
...
void setPan(int16_t panv){
		
		//pan=mypan
		factor_for_channelX[3]=sqlw[panv]>>7;
		factor_for_channelX[2]=sqlw[16383-(panv)]>>7;
		//Serial.println(factor_for_channelX[3]);
		//Serial.println("test");
	}
    virtual bool begin(AudioInfo info) override {
...
void setFilterType(char ftype){
 ...
 // void keyOff() { adsrxpfilter->keyOff();}
    ...
    T IRAM_ATTR readSample() override {
		...
		if (lrsamp==false){
      // at end deactivate output
	  if (mode==3) {start_Index=0;end_Index=4194304;loop_start=0;loop_end=4194304;}
	  if (loop_mode==ONESHOT){
      if (sound_index > end_Index) {
        // LOGD("reset index - sound_index: %d, table_length: %d",sound_index,table_length);
        sound_index = 0;
      ...
      if (sound_index < start_Index) {
	   ...
	   if (mode==4){
		      ...
		      if ((sound_indexb > 4194304)) {
		  
			   int div1 = (sound_indexb-4194304);
			   int div2 = (4194304);
			   if (div1!=0 and div2!=0){
			   sound_indexb=(div1%div2);
      ...
      if (this->is_running) {
		...
		prev_o2=result;
		  ...
		  if ((mode==0) || (mode==4)) {
			 ...
			 if (next_index>(loop_end>>SHIFTER)){
				 next_index=(loop_start>>SHIFTER)+offset;
			 }*/
			   int res_2=table[my_index+1]<<7;
		...
		prev_o2=in1;
		 ...
		 tmp=loop_end-((loop_start-tmp)%(loop_end-loop_start));}
		  ...
		  if (mode==3) {
			  //int my_index=(sound_index>>SHIFTER);
			    o2_index=o2i;
			  result=(sound_index>>8);
			 if (result>(o2_index<<6)){
				 result=16383;
			 } else {result=0;}
		...
		if ((mode==4)){
			int buf=result;
			   o2_index=o2i;
			  result=(sound_indexb>>8);
			 if (result>(o2_index<<6)){
				 result=16383;
			 } else {result=0;}
			 
			result=(buf>>1)+((result>>1));
			
			
				sound_indexb+=((tii*(sqrtune+64))>>5);
			}
	 
		 //Serial.println(cacheb);
		...
		divider=1;
		 ...
		 //cache = (adsrxp->tick())>>8;
	   	if ((prev_val[2]>0) && (input<0)) {zcd=true;}
					if ((prev_val[2]<0) && (input>0)) {zcd=true;}
					if (zcd==true){volume=volumeb;factor_for_channelX[0]=factor_for_channelX[2];factor_for_channelX[1]=factor_for_channelX[3];};
					cacheb=cache;
					  calc=(((cacheb * input)>>15)*volume)>>14;
					  
					 if ((mode==3)||(mode==4)){
	  int in1=o2->readSample();
		
	calc=((calc)+((in1*volume)>>16));
	   ...
	   calc+=(fade_test);
	   if (fade_test>=0){
	   fade_test-=fade_test_dec;}*/
			
					  //int64_t calc=((input)*volume)>>14;
					 
					 // if (adsrxp->isActive()){Serial.println(cacheb);}
					 result = (calc * factor_for_channelX[0])>>5;
					  //result=calc;
					 //result=result>>1;
	prev_val[2]=input;
	   lrsamp=true;} else {result=0;}
	
	   } else {
		   result = (calc * factor_for_channelX[1])>>5;
		   //result=calc;
		   lrsamp=false;
	   }
...
void setRZ(bool rzv){
  ...
  //cache = (adsrxp->tick());
  factor_for_channelX[0]=factor_for_channelX[2];factor_for_channelX[1]=factor_for_channelX[3];
  qn_pitch=0;cacheb=cache;volume=volumeb;
  
  adsrxpfilter->keyOn(tgt);
  sound_index=start_Index;
  pressed=true;
 // Serial.println(index_increment);
  }

  void keyOff() { adsrxp->keyOff();adsrxpfilter->keyOff();pressed=false; }
...
//bool zcd=false;
  ...
  int16_t o_release;
  int16_t factor_for_channelX[4]={0,0,0,0};
int16_t result = 0;
    int sound_index = 0;
    int sound_indexb = 0;
	int8_t sqrtune=64;
...
template <class T>
class WomprisXMSynthVoice : public SoundGenerator<T> {
  public:

    WomprisXMSynthVoice(int repeat=0, bool setInactiveAtEnd=true, size_t startIndex=0) {
        TRACED();
        this->max_repeat = repeat;
        this->inactive_at_end = setInactiveAtEnd;
        this->sound_index = startIndex;
...
/*
 ...
 void setO2(SoundGenerator<T> *generator){
  ...
  if (this->is_running) {
		...
		if (sound_index>loop_end){
			sound_index=loop_start;
		}
		//splits sample processing time in half for efficient dual and chorus programs in economical wompris g1 implementations
		if (halfsamp==false)
		{
			halfsamp=true;
			sound_index+=(tii)<<2;
		mysamp= table[sound_index>>6];} else {halfsamp=false;}
		return mysamp;
	} else {return 0;}
...
void setRZ(bool rzv){
    ...
    // step size the sound index is incremented (default = 1)
    void setIncrement(int inc){
        index_increment = inc>>6;
		porta=inc;
		ic2=((icache>>2)*(index_increment+pitch_bend>>2))>>13;
		tii=(index_increment+pitch_bend)+(ic2);
		//icache=inc;
    }
	...
	void setStart(int index){
        start_Index = (uint64_t)index<<(uint64_t)6;
    }
	
	void setEnd(int index){
        end_Index = (uint64_t)index<<6;
    }
	
	void setLoopStart(int index){
        loop_start = (uint64_t)index<<6;
    }
	void setO2Index(int16_t ind){
		o2i=ind;
	}
	void setLoopEnd(int index){
        loop_end = (uint64_t)index<<6;
    }
...
template <class T>
class WomprisSynthVoiceStrippedFlash : public SoundGenerator<T> {
  public:

    WomprisSynthVoiceStrippedFlash() = default;
    /**
     * @brief Construct a new Generator From Array object
     * 
     * @tparam array array of audio data of the the type defined as class template parameter 
    ...
    template  <size_t arrayLen> 
    WomprisSynthVoiceStrippedFlash(const int8_t(&array)[arrayLen], int repeat=0, bool setInactiveAtEnd=true, size_t startIndex=0) {
        TRACED();
        this->max_repeat = repeat;
        this->inactive_at_end = setInactiveAtEnd;
        this->sound_index = startIndex;
...
/*
    ~WomprisSynthVoiceStrippedFlash(){
        if (owns_data){
            delete[] table;
        }
    }*/
    ...
    T IRAM_ATTR readSample() override {
      ...
      //LOGD("index: %d - active: %d", sound_index, this->active);
		  ...
		  if (mode==1 ){
			...
			//  int res_1=table[my_index];
			  
			  // int res_2=table[my_index+1];
			// int frac=sound_index & 4095;
			 if (halfsample==true) {
				  int64_t my_index=(sound_index>>SHIFTER);
				 result=table[my_index];halfsample=false;
		...
		prev_val[0]=result;
			 ...
			 // }
			  //
} else {halfsample=true;result=prev_val[0];}
			  }
		  if (mode==0) {
halfsample=true;
divider=1024;
			  int64_t my_index=(sound_index>>SHIFTER)+offset;
			 // int my_indexb;
			 result=table[my_index];
			  //int res_1=table[my_index];
			  
			  /*int res_2;
			  if (my_index+1<loop_end){
			  res_2=table[my_index+1];} else {
				 res_2=table[loop_start];
				  
			  }
			 int frac=sound_index & 4095;
			  result=(res_1+((frac*(res_2-res_1))>>12))>>1;*/
			  //result=table[my_index];
			  //int res_2;
			  //if (reverse==false){
			 // if ((my_index+1)>(loop_end>>SHIFTER)+offset){res_2=0;}else{res_2=table[my_index+1]<<7;}} else {
		...
		prev_o2=in1;
		  ...
		  //tmp=loop_start;}
		 tmp=loop_end-((loop_start-tmp)%(loop_end-loop_start));}
		   int64_t my_index=(tmp>>SHIFTER)+offset;
			  int my_indexb;
			  int res_1=table[my_index]<<7;
...
newNote=false;
		...
		divider=1;
		 ...
		 //cache = (adsrxp->tick())>>8;
	   	if ((prev_val[2]>0) && (input<0)) {zcd=true;}
					...
					if (wait==divider){cache = (adsrxp->tick());wait=0;}
					if (zcd==true){volume=volumeb;cacheb=cache;factor_for_channelX[0]=factor_for_channelX[2];factor_for_channelX[1]=factor_for_channelX[3];};
					//int64_t calc=resultcache;
				//if (prev_val[2]!=input){
					int64_t calc;
				 if (halfsample==true){calc=(((cacheb * prev_val[2])>>2)*volume)>>4;} else {
	...
	prev_val[2]=input;
	   lr_sample=true;
	   resultcache=result;
	   return (result*factor_for_channelX[0])>>5;
	   
	   } else {
		   lr_sample=false;
		   return (resultcache*factor_for_channelX[1])>>5;
...
void setStart(int index){
	...
	uint8_t note;
		...
		void setVolume(int16_t vol){volumeb=vol;}
  void setAttackAmpRate(int16_t a) { adsrxp->setAttackRate(a);if (a==255){adsrxp->setAttackRate(32768);} }

  int16_t attackAmpRate() { return adsrxp->attackRate(); }

  void setDecayAmpRate(int16_t d) { adsrxp->setDecayRate(d); if (d==255){adsrxp->setDecayRate(2097152);}}
  ...
  void setReleaseAmpRate(int16_t r) { adsrxp->setReleaseRate(r); if (r==255){adsrxp->setReleaseRate(131072);};o_release=r; //divider=1;
  }
  ...
  adsrxp->keyOn(255);
  //divider=1;  
  cache = (adsrxp->tick());cacheb = cache;//m_osc->begin();
  qn_pitch=0;volume=volumeb;
  factor_for_channelX[0]=factor_for_channelX[2];factor_for_channelX[1]=factor_for_channelX[3];
  sound_index=start_Index;
 // adsrxpfilter->keyOn(tgt);
  trueBegin();
