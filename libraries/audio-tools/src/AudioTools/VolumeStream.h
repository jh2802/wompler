...
class VolumeStream : public AudioStream {
    public:
        ...
        virtual size_t readBytes(uint8_t *buffer, size_t length) override { 
            ...
            if (is_active) {
				if (int_mode==false){
				applyVolume(buffer,length);} else {applyVolumeX((int16_t*)buffer,length);}
        ...
        virtual size_t write(const uint8_t *buffer, size_t size) override {
            ...
            if (is_active) {
				if (int_mode==false){
				applyVolume(buffer,size);} else {applyVolumeX((int16_t*)buffer,size/2);}
...
//cleanupX();
    ...
    protected:
		...
		/*void cleanupX() {
          if (factor_for_channelX!=nullptr) {
            delete []factor_for_channelX;
            factor_for_channelX = nullptr;
          } 
          if (volume_values!=nullptr) {
            delete []volume_values;
            volume_values = nullptr;
		...
		uint8_t factorForChannelX(int channel){
		 ...
		 void applyVolumeX(int16_t* data, size_t size){
            for (size_t j=0;j<size;j++){
                ...
                //data[j] = (factor_for_channelX[j & 1]* data[j])>>7;
				
                //int16_t result = (factor_for_channelX[j & 1]* data[j])>>7;
                //int result = (factor_for_channelX[j%2]* data[j])>>7;
               /* if (!info.allow_boost){
                    if (result>max_value) result = max_value;
                    if (result<-max_value) result = -max_value;
