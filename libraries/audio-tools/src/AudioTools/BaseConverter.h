...
class ChannelEnhancer  {
    public:
		...
		void setPan(int16_t panv){
			factor_for_channelX[3]=sqlw[panv];
			factor_for_channelX[2]=sqlw[16383-(panv)];
        ...
        size_t convert(uint8_t*target, uint8_t*src, size_t size) {
            ...
            for(int i=0; i < frame_count; i++){
                ...
                for (int j=0;j<from_channels;j++){
                    value = *source++;
					zcd=false;
					if ((prev_val>=0) && (value<=0)) {zcd=true;}
					if ((prev_val<=0) && (value>=0)) {zcd=true;}
					if (zcd==true){
						factor_for_channelX[0]=factor_for_channelX[2];
						factor_for_channelX[1]=factor_for_channelX[3];
					
					};
					
                    *result++ = (int32_t)(value * factor_for_channelX[0])>>12;
                ...
                for (int j=from_channels;j<to_channels;j++){
                    *result++ = (int32_t)(value * factor_for_channelX[1])>>12;
                    result_size += sizeof(T);
                }                
            }
            return result_size;
